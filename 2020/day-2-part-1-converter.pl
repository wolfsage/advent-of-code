...
# Convert our input into something our dc script can understand (since dc
# isn't so great at working with strings)
#
# For example:
#   6-7 w: wwhmzwtwwk
# becomes:
# 6 7 119 119 119 104 109 122 119 116 119 119 107 13

while (my $line = <DATA>) {
  chomp($line);

  my ($h, $l, $needle, $haystack) = $line =~ /^(\d+)-(\d+)\s+(\w):\s*(\w+)\s*$/;
  die "Bad input? (line: $line)!\n" unless $haystack;

  my @haystack = map { ord($_) } split //, $haystack;
